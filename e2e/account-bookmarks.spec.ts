...
test.describe('Account bookmarks across browser contexts', () => {
    ...
    test('save on artist page, see it on another device, search and remove from the real profile', async ({ page, browser, baseURL }) => {
        if (!baseURL || !['localhost', '127.0.0.1'].includes(new URL(baseURL).hostname)) {
            throw new Error('This write test is restricted to a local app connected to dev.');
        }
        ...
        try {
            ...
            await page.reload();
            await expect(page.getByRole('button', { name: 'Bookmark', exact: true })).toHaveAttribute('aria-pressed', 'false');
            ...
            const cleanup = await fetchAsUser(page, '/api/bookmarks', { method: 'DELETE', headers, body: JSON.stringify({ artistId }) });
