...
jest.mock('@/server/utils/queries/externalApiQueries', () => ({
  getSpotifyHeaders: (...args: any[]) => mockGetSpotifyHeaders(...args),
  getSpotifyArtist: (...args: any[]) => mockGetSpotifyArtist(...args),
}));

// ── SearchBar dependency mocks ────────────────────────────────────────────────
...
jest.mock('@tanstack/react-query', () => ({
  ...
  QueryClientProvider: function MockQCP({ children }: any) { return <>{children}</>; },
...
jest.mock('@/components/ui/input', () => ({
  Input: function MockInput(props: any) { return <input {...props} />; },
}));
...
describe('AddArtistPage – ?spotify= URL param', () => {
  ...
  it('renders AddArtistContent when authenticated with a spotify param', async () => {
    ...
    mockGetSpotifyArtist.mockResolvedValue({
      data: { id: 'spotify-abc', name: 'Test Artist' },
      error: null,
    });
  ...
  it('fetches the Spotify artist with the provided spotify ID', async () => {
    mockGetServerAuthSession.mockResolvedValue({ user: { id: 'user-1' }, expires: '2099-01-01' });
    mockGetSpotifyArtist.mockResolvedValue({
      data: { id: 'spotify-xyz', name: 'Another Artist' },
      error: null,
    });
