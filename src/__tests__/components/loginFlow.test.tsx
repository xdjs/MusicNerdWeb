...
jest.mock('@/components/ui/button', () => {
  ...
  const MockButton = React.forwardRef(
    ({ children, onClick, disabled, id, type, ...rest }: any, ref: any) => (
      <button ref={ref} onClick={onClick} disabled={disabled} id={id} type={type} {...rest}>
        {children}
      </button>
    )
  );
...
describe('Login flow', () => {
  ...
  describe('onComplete callback – fresh login', () => {
    it('calls NextAuth signIn with privy provider after Privy onComplete', async () => {
    ...
    it('does NOT call NextAuth signIn when wasAlreadyAuthenticated is true and NextAuth is in sync', async () => {
      mockPrivyState.authenticated = true;
