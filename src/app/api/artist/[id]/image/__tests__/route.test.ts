...
it('falls back to providers when legacy custom URLs are invalid',async()=>{for(const customImage of ['garbage','//example.com/photo.jpg','javascript:alert(1)','http://cdn.example.com/photo.jpg','https://[broken','https://user:password@example.com/photo.jpg']){(db.query.artists.findFirst as jest.Mock).mockResolvedValue({id,customImage});const r=await get();expect(r.status).toBe(307);expect(r.headers.get('Location')).toBe('https://cdn.example.com/photo.jpg');}});
