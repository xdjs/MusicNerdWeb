...
export default function ProfileConcept({ user, showcase = false }: { user: Pick<User, 'id' | 'email' | 'wallet' | 'isAdmin' | 'isWhiteListed' | 'isHidden'>; showcase?: boolean }) {
  ...
  const collection = [...artists, ...sampleArtists];
  ...
  return <main className={`${styles.concept} mx-auto max-w-6xl px-5 sm:px-10 pb-16 text-foreground`}>
    ...
    <header className="flex flex-wrap items-center gap-3 sm:gap-5 py-6 sm:py-8">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 space-y-1 text-sm sm:text-base break-words">
    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_250px] lg:gap-10">
      ...
      <aside className="space-y-6">
        <section className="border border-[#ff75d8]/40 rounded-2xl p-5">
        ...
        <section className={`${styles.surface} rounded-2xl p-5`}><div className="flex items-baseline justify-between gap-2"><h2 className="font-semibold text-lg">Your contributions</h2><span className="text-xs text-muted-foreground">Sample</span></div><div className="grid grid-cols-2 gap-3 my-4">{[{label:'Approved', value:'approved', count:sampleContributions.filter(entry => entry.accepted).length}, {label:'Pending', value:'pending', count:sampleContributions.filter(entry => !entry.accepted).length}].map(item => <a key={item.value} href="#concept-history" onClick={() => setHistoryFilter({value: item.value})} className="rounded-xl bg-background p-3 hover:ring-1 hover:ring-pink-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-pink-400"><span className="block text-2xl font-semibold tabular-nums">{item.count}</span><span className="text-sm text-muted-foreground">{item.label}</span></a>)}</div><a href="#concept-history" onClick={() => setHistoryFilter({value: 'all'})} className="flex items-center justify-between text-sm font-medium py-2">View all contributions<ArrowUpRight size={15} /></a><Link href="/leaderboard" className="border-t border-border mt-2 pt-3 flex items-center justify-between text-sm text-muted-foreground">Community leaderboard<ArrowUpRight size={15} /></Link></section>
      </aside>
    </div>

    <section id="concept-history" className="mt-9 border-t border-border pt-7 [&_input]:bg-neutral-100 dark:[&_input]:bg-[#242424] [&_select]:bg-neutral-100 dark:[&_select]:bg-[#242424]"><p className="text-sm text-muted-foreground mb-4">24 sample contributions · Fictional activity for this design preview; your real history is unchanged.</p><UserEntriesTable concept statusFilter={historyFilter} sampleEntries={sampleContributions} artistImages={Object.fromEntries(artists.filter(artist => artist.imageUrl).map(artist => [artist.artistName, artist.imageUrl!]))} /></section>

    <Dialog open={collectionOpen} onOpenChange={setCollectionOpen}><DialogContent className="!top-auto !bottom-0 !translate-y-0 w-full max-w-2xl h-[85dvh] flex flex-col gap-4 rounded-t-3xl rounded-b-none sm:!top-1/2 sm:!bottom-auto sm:!-translate-y-1/2 sm:rounded-2xl bg-background/95 backdrop-blur-2xl p-5 sm:p-6">
    ...
    <Dialog open={sharingLink} onOpenChange={setSharingLink}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>Who is the link for?</DialogTitle><DialogDescription>Choose an artist, then use “Add a link” in their Links section.</DialogDescription></DialogHeader><div className="space-y-2">{artists.filter(artist => !artist.sample).map(artist => <Link key={artist.artistId} href={`/artist/${artist.artistId}#mn-links`} className="flex items-center justify-between rounded-xl border border-border p-3 hover:bg-muted">{artist.artistName}<ArrowUpRight size={16} /></Link>)}</div><Link href="/" className="text-sm underline underline-offset-4">Find another artist</Link></DialogContent></Dialog>
