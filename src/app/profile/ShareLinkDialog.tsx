...
export default function ShareLinkDialog({ open, onOpenChange, title = "Share a link" }: { title?: string; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="mn-themed-dialog artist-link-panel max-h-[85dvh] w-[calc(100%_-_2rem)] max-w-md overflow-y-auto rounded-2xl border-white/15 bg-neutral-950/80 bg-gradient-to-br from-white/[0.08] via-transparent to-white/[0.02] p-5 text-foreground shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_24px_80px_rgba(0,0,0,0.45)] backdrop-blur-2xl backdrop-saturate-150 sm:rounded-2xl sm:p-6">
      <DialogHeader className="space-y-2 pr-7 text-left">
        ...
        <DialogDescription className="text-sm leading-relaxed text-muted-foreground">Find the artist you’d like to contribute to.</DialogDescription>
      </DialogHeader>
      <div className="relative">
        <Search aria-hidden="true" className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input aria-label="Search for an artist to share a link" placeholder="Search for an artist…" value={query} onChange={event => setQuery(event.target.value)} className="min-h-12 rounded-xl border-pastypink/60 bg-white/[0.04] pl-10 text-base text-foreground placeholder:text-muted-foreground focus-visible:ring-pastypink/30" />
      </div>
      ...
      {results.length > 0 && <ul className="divide-y divide-border">{results.map(artist => {
        ...
        return <li key={artist.id}><Link onClick={() => onOpenChange(false)} href={`/artist/${artist.id}#mn-links`} className="flex items-center justify-between gap-3 rounded-lg px-3 py-3 text-foreground hover:bg-pastypink/10 focus-visible:outline focus-visible:outline-pink-400">{identity}<ArrowUpRight size={16} className="shrink-0 text-muted-foreground" aria-hidden="true" /></Link></li>;
      })}</ul>}
      <p className="text-xs text-muted-foreground">Choose an artist, then select “Add a link” in their Links section.</p>
