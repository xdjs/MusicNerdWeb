  ...
  return (
    <div className="max-w-3xl mx-auto mt-10">
      ...
      <div className="text-center mb-4">
        <h2 className="text-2xl font-semibold text-text-heading outline-none">Your Artist Data Entry</h2>
      </div>
      
      <Card className="border-2 border-border-card shadow-none">
        {/* Mobile: Single scrollable container for header and table */}
        <div className="overflow-x-auto min-w-full max-w-full">
          {/* Table Header */}
          <div className="bg-bg-table-header p-0 rounded-t-md border-b-2 border-border-card min-w-full">
            <div className="grid grid-cols-[1fr_1fr_2fr_3fr_0.8fr_1fr] sm:grid-cols-[1fr_1fr_2fr_2.5fr_0.8fr_1fr] text-white">
              <div
                className="text-center cursor-pointer select-none py-3 px-1 border-l border-t border-bg-table-header rounded-tl-md"
              ...
              <div className="text-center py-3 px-1 border-t border-bg-table-header">
                <span className="whitespace-nowrap">Time</span>
              </div>
              <div className="text-left py-3 px-4 border-t border-bg-table-header">
              ...
              <div className="text-left py-3 px-4 border-t border-bg-table-header min-w-[160px]">
                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                  <span className="whitespace-nowrap text-sm sm:text-base">Entry Type</span>
                  <select
                    value={filter}
                    onChange={(e) => setFilter(e.target.value)}
                    className="border border-gray-300 rounded-md p-1 text-xs h-6 w-full sm:w-16 text-text-heading bg-white"
                  >
              ...
              <div
                className="text-center py-3 px-2 cursor-pointer select-none border-t border-r border-bg-table-header rounded-tr-md"
                onClick={() =>
                  setStatusSort((prev) =>
          ...
          {/* Table Body */}
          <div className="p-0 border-b-2 border-border-card min-w-full">
            <Table className="min-w-full">
              <TableBody>
                {loading ? (
                  <TableRow className="bg-white hover:bg-white border-b border-border-card">
                    <TableCell colSpan={6} className="text-center py-4">
                      <div className="flex items-center justify-center gap-2">
                        <div className="animate-spin rounded-full h-4 w-4 border-2 border-bg-table-header border-t-transparent"></div>
                        <span>Loading...</span>
                      </div>
                    </TableCell>
                  </TableRow>
                  ...
                  (() => {
                    ...
                    return processed.slice(pageStart, pageEnd).map((entry) => {
                      ...
                      return (
                        <TableRow key={entry.id} className="bg-white hover:bg-white border-b border-border-card">
        ...
        {pageCount > 1 && (
          <CardFooter className="bg-bg-table-header border border-bg-table-header rounded-b-md flex justify-end items-center gap-4 p-3">
            <Button
              variant="outline"
              size="sm"
              disabled={page === 1}
              onClick={() => setPage((p) => p - 1)}
              className="bg-white text-bg-table-header border-white hover:bg-gray-100 hover:text-bg-table-header"
            >
              Prev
            </Button>
