...
export function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      {/* Text label - positioned on opposite side from button */}
      <div className="flex w-full px-2 text-xs font-medium text-white">
        <span className={`transition-opacity duration-300 w-full ${theme === "dark" ? 'opacity-100 text-left' : 'opacity-0'}`}>
          Dark Mode
        </span>
        <span className={`transition-opacity duration-300 w-full ${theme === "dark" ? 'opacity-0' : 'opacity-100 text-right'}`}>
          Light Mode
        </span>
      </div>
    </button>
