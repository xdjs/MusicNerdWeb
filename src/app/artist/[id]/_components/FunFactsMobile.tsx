  ...
  return (
    <div className="bg-white rounded-lg shadow-2xl p-4 space-y-4 overflow-x-hidden md:hidden">
      ...
      <div className="relative">
            ...
            {buttons.map(({ type, label, icon }) => (
              <Tooltip key={type} delayDuration={300}>
                <TooltipTrigger asChild>
                  <Button
                    variant="outline"
                    className="w-full flex items-center justify-center text-base font-semibold border-2 border-border-card"
        ...
        {/* Overlay Fact Box */}
        {fact && (
                          <div className="absolute inset-0 flex flex-col bg-white rounded-lg border-2 border-border-card shadow-lg overflow-y-auto overflow-x-hidden pt-0.5 pb-0.5 pr-0.5 pl-4">
            {/* Close button */}
            <button
              className="sticky top-0.5 ml-auto mr-0.5 flex h-6 w-6 items-center justify-center text-xl font-bold text-white border-2 border-border-card rounded-md bg-border-card hover:bg-bg-button-hover focus:outline-none leading-none z-10"
