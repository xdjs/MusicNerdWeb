...
export default function HeaderPhotoPosition({ artistId, imageUrl, position, onChange, onClose }: Props) {
    ...
    useEffect(() => { dragSurface.current?.focus(); }, []);
    ...
    return <div className="absolute inset-0 z-20" onKeyDown={e => { if (e.key === 'Escape' && !saving) { e.stopPropagation(); cancel(); } }}>
        <div ref={dragSurface} role="slider" aria-label="Photo position" aria-orientation="vertical"
            aria-valuemin={0} aria-valuemax={100} aria-valuenow={draft} aria-valuetext={`${draft}% from top`}
            aria-describedby="photo-position-keyboard-help" aria-disabled={saving} tabIndex={saving ? -1 : 0}
            onKeyDown={e => {
                if (saving) return;
                const next = e.key === 'ArrowUp' ? draft + 1 : e.key === 'ArrowDown' ? draft - 1 : e.key === 'Home' ? 0 : e.key === 'End' ? 100 : null;
                if (next !== null) { e.preventDefault(); change(next); }
            }}
            className={`absolute inset-0 touch-none select-none focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-4 focus-visible:outline-pastypink ${saving ? '' : 'cursor-grab active:cursor-grabbing'}`}
            onPointerDown={e => {
        ...
        <p id="photo-position-keyboard-help" className="sr-only">Use the up and down arrow keys to move the photo. Press Escape to cancel.</p>
