...
Also say what kind of page each source is:
- "coverage": someone writing or talking about the artist or their work: an interview, review, article, feature, profile piece or podcast episode.
- "listing": a directory, catalogue, database, discography, lyrics, store or streaming page that lists the artist or their releases.
- "own": the artist's own website or one of their own accounts.
Return one verdict for every source, with a short reason.`;

const VERDICTS = z.object({
    verdicts: z.array(z.object({ index: z.number().int(), aboutArtist: z.boolean(), kind: z.enum(["coverage", "listing", "own"]), reason: z.string() })),
});

/**
 * The research suite's one judge: is each source research kept about this artist, and
 * is it coverage, a listing or the artist's own page? One
...
export async function judgeKeptSources(researchCase: ResearchCase, sources: KeptSource[]): Promise<SourceVerdict[]> {
    ...
    return sources.map((s, i) => {
        ...
        return v
            ? { url: s.url, aboutArtist: v.aboutArtist, kind: v.kind, reason: v.reason }
            : { url: s.url, aboutArtist: false, kind: "listing" as const, reason: "no verdict from the judge" };
    });
}
